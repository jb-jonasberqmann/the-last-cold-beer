...
import { startGame, assignPlayerToTeam, renameTeam } from "@/lib/game/actions";
...
export default function LobbyPage({ params }: Props) {
  ...
  const fetchLobby = useCallback(async () => {
    ...
    if (data.game?.status === "active" && session) {
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          {(["team-a", "team-b"] as TeamId[]).map((teamId) => {
            ...
            return (
              <div
                key={teamId}
                className="rounded-xl bg-stone-800/50 border border-stone-600 p-3"
              >
